...
test('admin sees companies and pending requests', async () => {
  ...
  createAccessRequests(db).createRequest({
    companyName: 'IBM',
    contactName: 'James',
    email: 'james@ibm.com',
  });
...
test('approve provisions and emails the CR', async () => {
  ...
  const r = createAccessRequests(db).createRequest({
    companyName: 'IBM',
    contactName: 'James',
    email: 'james@ibm.com',
  });
  ...
  const compEnts = db
    .prepare(
      "SELECT app FROM app_entitlements WHERE principal_type='company' AND principal_id=? ORDER BY app",
    )
    .all(company.id);
  assert.deepEqual(
  ...
  const userEnts = db
    .prepare(
      "SELECT app FROM app_entitlements WHERE principal_type='user' AND principal_id=? ORDER BY app",
    )
    .all(user.id);
  assert.deepEqual(
