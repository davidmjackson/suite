...
test('grantEntitlement inserts; re-grant updates terms (upsert on unique key)', () => {
  ...
  let row = db
    .prepare(
      "SELECT * FROM app_entitlements WHERE app='raid' AND principal_type='user' AND principal_id='u1'",
    )
    .get();
...
test('grantEntitlement rejects invalid principal type', () => {
  ...
  const ent = createEntitlements(db);
  assert.throws(
    () => ent.grantEntitlement({ app: 'raid', principalType: 'robot', principalId: 'x' }),
...
test('revokeEntitlement suspends the grant', () => {
  ...
  const row = db
    .prepare(
      "SELECT status FROM app_entitlements WHERE app='signal' AND principal_type='user' AND principal_id='u1'",
    )
    .get();
...
test('no grant -> denied', () => {
  ...
  const ent = createEntitlements(db);
  seedUser(db, 'u1', 'a@b.c');
  assert.deepEqual(ent.resolveEntitlement('u1', 'raid'), {
    entitled: false,
    principal: null,
    quota: null,
  });
...
test('company-level quota grant -> remaining = limit - usage', () => {
  ...
  db.prepare(
    "INSERT INTO app_usage (app,principal_type,principal_id,period_key,count) VALUES ('raid','company',?,?,13)",
...
test('consume increments the counter and reports remaining', () => {
  ...
  const count = db
    .prepare(
      "SELECT count FROM app_usage WHERE app='raid' AND principal_type='user' AND principal_id='u1'",
    )
    .get().count;
