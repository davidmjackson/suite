...
function insertToken(
  db,
  tok,
  { email = 'a@b.c', returnTo = null, expiresAt = now() + 60_000, consumedAt = null } = {},
) {
  db.prepare(
    `INSERT INTO magic_link_tokens (token,email,return_to,created_at,expires_at,consumed_at) VALUES (?,?,?,?,?,?)`,
...
test('GET with valid token renders a confirm page and does NOT consume the token', async () => {
  ...
  assert.equal(
    db.prepare('SELECT consumed_at FROM magic_link_tokens WHERE token = ?').get(tok).consumed_at,
    null,
  );
...
test('every allowed app domain resolves to a launch, so no accepted return_to is dropped', async () => {
  ...
  assert.ok(
    domains.length >= 2,
    'the allowlist fixture must hold the real domains to guard anything',
  );
