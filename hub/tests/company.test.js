...
test('owner can create a team', async () => {
  ...
  assert.equal(res.status, 302);
  const t = db
    .prepare('SELECT * FROM teams WHERE company_id=? AND name=?')
    .get(company.id, 'Squad B');
  assert.ok(t);
});
...
test('GET team page is 404 for a team in another company', async () => {
  ...
  const res = await request(app)
    .get(`/company/acme/teams/${otherTeam.id}`)
    .set('Cookie', cookie(sid));
  assert.equal(res.status, 404);
});
...
test('remove a team member', async () => {
  ...
  org.addTeamMember({
    userId: 'u1',
    teamId: org.createTeam({ companyId: company.id, name: 'Squad' }).id,
    role: 'member',
  });
  const t = db
...
test('owner can grant then revoke RAID for a member', async () => {
  ...
  let ent = db
    .prepare(
      "SELECT quota_limit,status FROM app_entitlements WHERE app='raid' AND principal_type='user' AND principal_id='mem'",
    )
    .get();
  ...
  ent = db
    .prepare(
      "SELECT status FROM app_entitlements WHERE app='raid' AND principal_type='user' AND principal_id='mem'",
    )
    .get();
...
test('owner can grant Signal (unlimited) for a member', async () => {
  ...
  const ent = db
    .prepare(
      "SELECT quota_limit,status FROM app_entitlements WHERE app='signal' AND principal_type='user' AND principal_id='mem'",
    )
    .get();
  assert.equal(ent.status, 'active');
...
test('owner of one company cannot toggle a member of another company (404, cross-tenant)', async () => {
  ...
  const ent = db
    .prepare(
      "SELECT 1 FROM app_entitlements WHERE principal_type='user' AND principal_id='outsider'",
    )
    .get();
