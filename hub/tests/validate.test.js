...
test('validate: on success replaces req.body with parsed/coerced data and calls next()', async () => {
  const schema = z.object({
    email: z.preprocess((v) => String(v).trim().toLowerCase(), z.string()),
  });
  const { req, nextErr } = await run(validate(schema), {
    body: { email: '  A@B.COM ', extra: 'x' },
  });
...
test('validate: on failure with onInvalid calls it instead of next()', async () => {
  const schema = z.object({ email: z.string().email() });
  let called = false;
  const onInvalid = (req, res) => {
    called = true;
    req._resolvedByOnInvalid = true;
  };
