...
function seedLegacy(db) {
  ...
  db.prepare(
    "INSERT INTO companies (id,name,slug,status,created_at) VALUES (?,?,?, 'active', ?)",
  ...
  for (const [app, lim, per] of [
    ...
    db.prepare(
      "INSERT INTO app_entitlements (id,app,principal_type,principal_id,status,quota_limit,quota_period,granted_by,granted_at) VALUES (?,?, 'company', ?, 'active', ?,?, NULL, ?)",
...
test('migration 004 collapses admin to member', () => {
  ...
  const role = db
    ...
    .get(adminId, companyId).role;
  assert.equal(role, 'member');
  const ownerRole = db
    .prepare('SELECT role FROM company_members WHERE user_id=? AND company_id=?')
    .get(ownerId, companyId).role;
...
test('migration 004 re-homes company signal/raid to owners (user-level) and suspends company grants', () => {
  ...
  const { ownerId, adminId, companyId } = seedLegacy(db);
  runMigration(db);
  const ownerSignal = db
    .prepare(
      "SELECT * FROM app_entitlements WHERE app='signal' AND principal_type='user' AND principal_id=? AND status='active'",
    )
    .get(ownerId);
  const ownerRaid = db
    .prepare(
      "SELECT * FROM app_entitlements WHERE app='raid' AND principal_type='user' AND principal_id=? AND status='active'",
    )
    .get(ownerId);
  ...
  const memberSignal = db
    .prepare(
      "SELECT 1 FROM app_entitlements WHERE app='signal' AND principal_type='user' AND principal_id=? AND status='active'",
    )
    .get(adminId);
...
test('migration 004 is idempotent (re-running is a no-op)', () => {
  ...
  runMigration(db);
  runMigration(db);
  const ownerRaidRows = db
    .prepare(
      "SELECT COUNT(*) AS n FROM app_entitlements WHERE app='raid' AND principal_type='user' AND principal_id=?",
    )
    .get(ownerId).n;
