...
function fakeSender(calls) {
  return { sendAccessRequestNotification: async (a) => { calls.push(a); } };
}
...
test("POST /request notifies the operator when ADMIN_EMAIL is set", async () => {
  ...
  const res = await request(app).post("/request").type("form").send({
    company_name: "IBM", contact_name: "James", email: "james@ibm.com",
    job_title: "SM", team_size: "11-50", apps: ["poker", "retro"], message: "keen",
  });
  assert.equal(res.status, 200);
...
test("POST /request does not notify on honeypot or invalid submissions", async () => {
  ...
  const { mountRequest } = await import("../routes/request.js?t=" + Date.now());
  mountRequest(app, { emailSender: fakeSender(calls) });
  await request(app).post("/request").type("form").send({ company_name: "X", contact_name: "Y", email: "x@y.co", website: "spam" });
  await request(app).post("/request").type("form").send({ company_name: "X", contact_name: "Y", email: "bad" });
  assert.equal(calls.length, 0);
});
...
test("POST /request still succeeds if the notification email throws", async () => {
  ...
  const { mountRequest } = await import("../routes/request.js?t=" + Date.now());
  mountRequest(app, { emailSender: { sendAccessRequestNotification: async () => { throw new Error("resend down"); } } });
  const res = await request(app).post("/request").type("form").send({
    company_name: "Acme", contact_name: "Jo", email: "jo@acme.co",
  });
  assert.equal(res.status, 200);
  assert.match(res.text, /received/i);
...
test("request notification email renders and escapes user input", async () => {
  ...
  const html = await renderAccessRequestNotificationEmail({
    request: { companyName: "A&B <x>", contactName: "Jo", email: "jo@a.co", jobTitle: null, teamSize: null, apps: [], message: "<script>alert(1)</script>" },
