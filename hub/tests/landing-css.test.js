...
test('the melon mirror still matches shared/theme — the promo page uses the live token', () => {
  ...
  assert.match(
    rule('.appcard[data-app="sight"]').replace(/\s+/g, ' '),
    new RegExp(`--accent:\\s*${melon.replace(/[()]/g, '\\$&')}`),
  );
  const tag = rule('.tag-sight').replace(/\s+/g, ' ');
  assert.ok(
    tag.includes(melonwash),
    `.tag-sight background must mirror --melonwash (${melonwash})`,
  );
...
test('the plum mirror still matches shared/theme', () => {
  ...
  assert.ok(
    rule('.appcard[data-app="plan"]').includes(plum),
    `plan accent must mirror --plum (${plum})`,
  );
