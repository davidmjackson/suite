...
const run = db.transaction(() => {
  ...
  db.prepare("DELETE FROM app_entitlements WHERE principal_type = 'company' AND principal_id = ?").run(company.id);
...
console.log(`Deleted company '${company.name}' (slug=${slug}, id=${company.id}) and its hub-side dependents.`);
db.close();
