...
const expiredError = (res) =>
  res.status(400).render("error", { title: "Link expired", message: "This sign-in link is expired or has already been used.", backHref: "/login" });
...
export function mountMagic(app) {
  ...
  app.get("/auth/magic", (req, res) => {
    ...
    const row = db.prepare("SELECT consumed_at, expires_at FROM magic_link_tokens WHERE token = ?").get(token);
