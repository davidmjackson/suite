...
export function mountAdmin(app, { emailSender } = {}) {
  ...
  app.post(
    ...
    validate(createUserSchema, {
      onInvalid: (req, res) =>
        res.status(400).render('error', { title: 'Bad request', message: 'Invalid email.' }),
    }),
    (req, res) => {
  ...
  app.post('/admin/users/:id/delete', requireSession, requireAdmin, (req, res) => {
    ...
    if (id === req.user.id)
      return res
        .status(400)
        .render('error', { title: "Can't delete self", message: 'Use another admin account.' });
    deleteUser(db, id);
    audit.log({
      userId: req.user.id,
  ...
  app.post('/admin/requests/:id/approve', requireSession, requireAdmin, async (req, res) => {
    ...
    audit.log({
      ...
      metadata: { company: result.company.slug, email: result.user.email },
      ip: req.ip,
    });
