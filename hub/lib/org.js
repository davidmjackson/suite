...
export function createOrg(db) {
  ...
  const getCompanyBySlug = (slug) =>
    db.prepare('SELECT * FROM companies WHERE slug = ?').get(slug) || null;
  const getTeam = (id) => db.prepare('SELECT * FROM teams WHERE id = ?').get(id) || null;
  const ownerCount = (companyId) =>
    db
      .prepare("SELECT COUNT(*) AS n FROM company_members WHERE company_id = ? AND role = 'owner'")
      .get(companyId).n;
  ...
  function setCompanyMemberRole({ userId, companyId, role }) {
    ...
    db.prepare('UPDATE company_members SET role=? WHERE user_id=? AND company_id=?').run(
      role,
      userId,
      companyId,
    );
  ...
  const removeCompanyMember = db.transaction(({ userId, companyId }) => {
    ...
    db.prepare('DELETE FROM company_members WHERE user_id=? AND company_id=?').run(
      userId,
      companyId,
    );
  });
  ...
  function listCompanyMembers(companyId) {
    ...
    `,
      ...
      .all(companyId)
      .map((r) => ({ ...r, hasLoggedIn: !!r.hasLoggedIn }));
  }
