...
test('three pass cards, each an <article> with pass label, title and in/out (copy verbatim, spec §6)', () => {
  ...
  for (const [, stage, title] of TP_PASSES) {
    assert.match(
      sec,
      new RegExp(`<p class="tp-pass">Pass 0\\d · ${stage}</p>`),
      `${stage} eyebrow`,
    );
...
test('card images use a bare page-relative filename, never the hub-served /illos/ path', () => {
  ...
  for (const [, src, alt] of imgs) {
    assert.doesNotMatch(
      src,
      /^(\/|https?:|\.\.\/)/,
      `${src} must be a page-relative filename, not absolute/external/traversal`,
    );
    assert.match(
      src,
      /^pass-0[123]-[a-z-]+\.svg$/,
      `${src} is a three-passes illustration in this dir`,
    );
...
test('the three SVGs are self-contained: no script, no external refs', () => {
  ...
  for (const [file] of TP_PASSES) {
    ...
    assert.doesNotMatch(
      svg,
      /<script|<!doctype|<!entity|xlink:href|\bhref\s*=|\son[a-z]+\s*=|<image\b|<foreignObject|url\(|data:/i,
      `${file} has no active/external content`,
    );
...
test('the pass labels use a section-local indigo, never the page-wide green --accent', () => {
  ...
  assert.match(
    css,
    /\.tp-pass \{[^}]*color: var\(--tp-accent\)/,
    'the pass label reads --tp-accent',
  );
...
test('canonical names the URL that actually serves 200', () => {
  ...
  assert.match(
    html,
    /<link rel="canonical" href="https:\/\/sprintsuite\.uk\/sprintsight-coming-soon\/intro\/">/,
  );
});
...
test(
  'the pinned ids still match data-strategy.md, the upstream source of truth',
  {
...
test('every cited id follows the documented {type}-{team}-s{sprint} convention', () => {
  ...
  const ids = [...js.matchAll(/"([a-z]+-(?:atlas|boreas|cygnus|draco)-s15[a-z-]*)"/g)].map(
    (m) => m[1],
  );
...
test('the OG image exists at 1200x630 and fits the size budget', () => {
  ...
  assert.deepEqual([...png.subarray(0, 8)], [137, 80, 78, 71, 13, 10, 26, 10], 'is a PNG');
