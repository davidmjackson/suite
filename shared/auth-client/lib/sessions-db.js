...
function createSessionsStore(dbPath) {
  ...
  `);
  const cols = db
    .prepare('PRAGMA table_info(app_sessions)')
    .all()
    .map((c) => c.name);
  if (!cols.includes('entitled'))
    db.exec('ALTER TABLE app_sessions ADD COLUMN entitled INTEGER NOT NULL DEFAULT 0');
  if (!cols.includes('teams'))
    db.exec("ALTER TABLE app_sessions ADD COLUMN teams TEXT NOT NULL DEFAULT '[]'");
  if (!cols.includes('company'))
    db.exec("ALTER TABLE app_sessions ADD COLUMN company TEXT NOT NULL DEFAULT 'null'");
  return {
    create({
      id,
      userId,
      ...
      const t = Date.now();
      db.prepare(
        `INSERT INTO app_sessions (id,user_id,central_session_id,created_at,last_validated_at,expires_at,entitled,teams,company) VALUES (?,?,?,?,?,?,?,?,?)`,
      ).run(
        id,
        userId,
        centralSessionId,
    ...
    get(id) {
      const row = db
        .prepare('SELECT * FROM app_sessions WHERE id = ? AND expires_at > ?')
        .get(id, Date.now());
      ...
      let company;
      try {
        company = JSON.parse(row.company || 'null');
      } catch {
        company = null;
      }
