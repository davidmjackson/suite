...
test('heartbeat returns ok for 200, expired for 404', async () => {
  const f = mockFetch({
    'POST /api/sessions/s1/heartbeat': async () => ({
      status: 200,
      json: async () => ({ ok: true }),
    }),
...
test('consume maps 200/402/403 to a verdict object and hits the app-scoped URL', async () => {
  ...
  const ok = createHubApi({
    baseUrl: 'https://hub',
    apiKey: 'k',
    appName: 'raid',
    fetchImpl: make(200, { ok: true, remaining: 5 }),
  });
  assert.deepEqual(await ok.consume('cs1'), { ok: true, remaining: 5 });
  assert.equal(calledUrl, 'https://hub/api/apps/raid/consume');

  const quota = createHubApi({
    baseUrl: 'https://hub',
    apiKey: 'k',
    appName: 'raid',
    fetchImpl: make(402, { ok: false, reason: 'quota_exceeded' }),
  });
  assert.deepEqual(await quota.consume('cs1'), { ok: false, reason: 'quota_exceeded' });

  const denied = createHubApi({
    baseUrl: 'https://hub',
    apiKey: 'k',
    appName: 'raid',
    fetchImpl: make(403, { ok: false, reason: 'not_entitled' }),
  });
